...
class PlaneManagementController:
    ...
    def get_planes(self):
        try:
            planes = Plane.get_all_planes(self.api_base_url)
    ...
    def add_plane(self, plane_data):
        if not is_airplane_image(plane_data["Picture"]):
            return False

        plane = Plane(
            plane_id=0,  # New plane, backend should assign ID
    ...
    def delete_plane(self, plane_id):
        try:
            plane = Plane.get_plane_by_id(self.api_base_url, plane_id)
            plane.delete(self.api_base_url)
            return True
        except Exception as e:
    ...
    def search_planes(self, keyword):
        ...
        all_planes = Plane.get_all_planes(self.api_base_url)
